    ...
    public function scopeForCheckoutSession($query, string $sessionId)
    {
        return $query->where(function ($query) use ($sessionId) {
            $query->where('checkout_session_id', $sessionId)
                ...
                ->orWhereHas('checkoutRetryOperations', fn ($query) => $query->where('target_session_id', $sessionId));
        });
    }

    public function checkoutRetryOperations()
    {
        return $this->hasMany(CheckoutRetryOperation::class, 'payment_id', 'payment_id');
    }
