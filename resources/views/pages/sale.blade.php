...
@endphp
        ...
        <a href="{{ route('product.show', ['id' => $product->product_id, 'return_to' => request()->fullUrl()]) }}" class="shoe-card" data-category="{{ strtolower($category) }}" data-price="{{ round($salePrice) }}" data-discount="{{ $discountPercent }}" aria-label="View {{ $product->product_name }}">
