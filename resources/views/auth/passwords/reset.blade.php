...
@section('content')
<!-- CINEMATIC RESET PASSWORD – SAME STYLE AS LOGIN/REGISTER -->
...
<body class="relative bg-black font-['Inter',sans-serif] overflow-x-hidden">
  ...
  <!-- RESET PASSWORD FORM – same card width as login -->
  <div class="relative z-20 min-h-screen flex items-center justify-center p-4 md:p-8">
    <div class="w-full max-w-[460px] bg-white/90 backdrop-blur-16 rounded-[48px] p-8 md:p-9 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.35),0_0_0_1px_rgba(255,255,255,0.7),0_0_0_2px_rgba(230,0,35,0.1),inset_0_1px_0_rgba(255,255,255,0.9)] transition-shadow duration-300 hover:shadow-[0_30px_60px_-15px_rgba(230,0,35,0.2),0_0_0_1px_rgba(255,255,255,0.8),0_0_0_2px_rgba(230,0,35,0.15),inset_0_1px_0_rgba(255,255,255,1)] glass-card-clean relative before:content-[''] before:absolute before:inset-0 before:rounded-[48px] before:bg-gradient-to-br before:from-white/30 before:to-transparent before:pointer-events-none">
      ...
      <div class="text-center mb-7">
        ...
        <p class="text-[#5a5a6e] font-medium text-sm tracking-wide">Create your new password</p>
      </div>

      <form method="POST" action="{{ route('password.update') }}" id="premiumResetForm">
        @csrf
        <input type="hidden" name="token" value="{{ $token }}">

        <!-- Email Field -->
        <div class="input-group-float @error('email') error @enderror" id="emailGroup">
          <input type="email" id="email" name="email" value="{{ $email ?? old('email') }}" placeholder=" " required autofocus>
        ...
        <!-- Submit Button -->
        <button type="submit" class="w-full bg-[#E60023] py-3.5 rounded-full font-extrabold text-sm text-white transition-all duration-200 hover:bg-[#C2001F] hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 shadow-[0_10px_22px_-10px_rgba(230,0,35,0.5)] relative overflow-hidden" id="premiumResetBtn">
    ...
    (function() {
      ...
      function initPasswordToggle(toggleBtn, passwordInput) {
        ...
        function updateVisibility() {
          ...
          toggleBtn.classList.toggle('visible', hasValue);
        }
        updateVisibility();
      ...
      initPasswordToggle(document.getElementById('togglePassword'), document.getElementById('password'));
      initPasswordToggle(document.getElementById('toggleConfirmPassword'), document.getElementById('password-confirm'));

      // ========== RIPPLE EFFECT ON BUTTON ==========
      const resetBtn = document.getElementById('premiumResetBtn');
      ...
      if(resetBtn) {
        resetBtn.addEventListener('click', (e) => {
          if(!resetBtn.disabled) addRipple(e, resetBtn);
        });
      }

      // ========== FORM VALIDATION ==========
      const form = document.getElementById('premiumResetForm');
      if(form) {
        form.addEventListener('submit', function(e) {
          ...
          if(password !== confirm) {
            ...
            let errDiv = confirmGroup.querySelector('.error-message');
            if(!errDiv) {
            ...
            resetBtn.innerHTML = `<i class="fas fa-circle-notch fa-spin"></i><span>RESETTING...</span>`;
          }
        });
      }
    })();
