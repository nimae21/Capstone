...
@section('content')
            ...
            <div class="stat-card p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-4xl font-black text-gray-800 mt-2">{{ $categories->total() }}</p>
                    </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($categories as $category)
                            <tr class="table-row">
                                ...
                                <td class="px-6 py-3 text-sm text-gray-500 max-w-xs truncate" title="{{ $category->category_description ?? 'No description' }}">
                                    {{ $category->category_description ?: '—' }}
                                </td>
                                <td class="px-6 py-3 text-right">
                                    <div class="flex gap-2 justify-end">
                                        <a href="{{ route('admin.categories.edit', $category->category_id) }}" class="btn-sm-blue">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                            Edit
                                        </a>
                                        <button type="button" onclick="openDeleteModal({{ $category->category_id }}, '{{ addslashes($category->category_name) }}')" class="btn-sm-red">
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
        <div class="bg-white p-6 rounded-2xl w-full max-w-sm shadow-2xl modal-enter">
            ...
            <h2 class="font-bold text-lg text-center">Delete Category?</h2>
    ...
    <div id="addSuccessModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 hidden">
        <div class="bg-white w-full max-w-sm p-6 rounded-2xl text-center shadow-2xl modal-enter">
            ...
            <h2 class="text-xl font-bold text-gray-800">Category Added!</h2>
            <p class="text-gray-600 mt-2" id="addSuccessMessage">Category has been created successfully.</p>
    ...
    <script>
        ...
        function openDeleteModal(categoryId, categoryName) {
            ...
            messageEl.textContent = `Delete category "${categoryName}"? This action cannot be undone.`;
        ...
        // ========== ADD CATEGORY FORM SUBMIT (with modal) ==========
        ...
        if (createForm) {
            createForm.addEventListener('submit', async function(e) {
                ...
                if (response.ok) {
                    ...
                    document.getElementById('addSuccessMessage').textContent = result.message || 'Category has been created successfully.';
