...
@foreach($item->payload['_images'] ?? [] as $descriptor)@php($path = is_array($descriptor) ? $descriptor['image_path'] : $descriptor)<a href="{{ Storage::disk(config('product_images.disk'))->url($path) }}" target="_blank" rel="noopener"><img src="{{ Storage::disk(config('product_images.disk'))->url($path) }}" alt="Proposed product image" width="90" height="90" loading="lazy" decoding="async" style="width:90px;height:90px;object-fit:contain"></a>@endforeach</details></td>
