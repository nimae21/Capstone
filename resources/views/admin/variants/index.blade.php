...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        ...
        <!-- Product Name Header -->
        <div class="mb-8">
            <h1 class="text-3xl font-bold gradient-title">Manage Variants for <span class="text-gray-800">{{ $product->product_name }}</span></h1>
            <p class="text-gray-500 mt-1">Add, edit, or remove size/color combinations for this product.</p>
        </div>

        <!-- Two Column Layout: Form + Stats / Tips -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <!-- Add Variant Form (centered inside its card) -->
            <div class="card-3d rounded-2xl p-6">
                <div class="flex items-center gap-2 mb-5">
                    <div class="w-1 h-5 bg-gradient-to-b from-red-600 to-black rounded-full"></div>
                    <h3 class="text-md font-bold gradient-title">Add New Variant</h3>
                </div>
                <form action="{{ route('admin.products.variants.store', $product->product_id) }}" method="POST" class="space-y-5">
                    @csrf
                    <div>
                        <label for="size" class="block text-sm font-semibold text-gray-700 mb-1">Size <span class="text-red-500">*</span></label>
                        <input type="text" name="size" id="size" placeholder="e.g., 42, M, XL, 10.5" required
                               class="input-premium">
                    </div>
                    <div>
                ...
                <div class="relative z-2">
                    <div class="flex items-center gap-3 mb-4">
                        <div class="p-2 bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl shadow-inner">
                            <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l5 5a2 2 0 01.586 1.414V19a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z"/></svg>
                        </div>
                        <h3 class="text-lg font-bold gradient-title">Variant Summary</h3>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-white/50 rounded-xl mb-3">
                        ...
                        <span class="text-2xl font-bold text-gray-800">{{ $product->variants->count() }}</span>
                    </div>
                    <div class="tip-3d flex items-center gap-2 text-sm p-3 rounded-lg transition-all mt-4">
                        ...
                        <span class="text-gray-600">Each variant (size+color) can have its own stock quantity. Use "Manage Stocks" to set inventory levels.</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-3d rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-lg font-bold gradient-title">Existing Variants</h3>
                <span class="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{{ $product->variants->count() }} variant(s)</span>
            </div>
            <div class="overflow-x-auto custom-scroll">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($product->variants as $variant)
                                <tr class="table-row-3d">
                                    <td class="px-6 py-3 text-sm text-gray-500">{{ $variant->product_variant_id }}</td>
                                    <td class="px-6 py-3 font-medium text-gray-800">{{ $variant->size }}</td>
                                    <td class="px-6 py-3 text-gray-600">{{ $variant->color }}</td>
                                    <td class="px-6 py-3 text-right">
                                        <div class="flex gap-2 justify-end">
                                            <a href="{{ route('admin.variants.edit', $variant->product_variant_id) }}" class="btn-sm-3d btn-sm-blue">
                                            ...
                                            <a href="{{ route('admin.stocks.index', $variant->product_variant_id) }}" class="btn-sm-3d btn-sm-green">
                ...
                @else
                    <div class="text-center py-12 text-gray-500">
                        No variants found for this product. Add your first variant using the form.
                    </div>
                @endif
            </div>
        </div>
